                    ...
                    @foreach ($groups as $group)
                        ...
                        @foreach ($group->versions as $version)
                            <div class="w-full lg:flex items-center">
                                <p>
                                    {{ $version->mod_version }}{{ $version->title ? (' (' . $version->title . ')') : '' }} for Minecraft {{ $version->mc_version }}, released on {{ $version->released_at->format('d/m/Y H:i:s') }}
                                </p>
                                <div class="flex-grow text-right py-1">
